...
int parse_configuration(libusb_config_descriptor_t *config, const uint8_t *buffer, int size)
{
    ...
    printf("size : %d\n\r", sizeof(buffer));
    printf("config->bNumInterfaces : %d\n\r", (uint8_t)config->bNumInterfaces);
//    usb_print_config_descriptor((const usb_config_desc_t *)buffer, print_usb_class_descriptors);
    ...
    for (int i = 0; i < config->bNumInterfaces; i++) {
        ...
        for (int alt = 0; alt < alt_interfaces; alt++) {
            ...
            printf("ifc_desc->bNumEndpoints : %d\n\r", ifc_desc->bNumEndpoints);
